# Flaro - Remote Desktop Control over WebRTC

Flaro is an advanced, ultra-low latency remote desktop control application built using WebRTC, React, and Electron. It allows users to control their desktop PC seamlessly from their mobile device with pixel-perfect touch mapping, gesture support, and dynamic zoom features.

## 🏗 Global Matchmaking Architecture

Flaro consists of three primary components that communicate via a global matchmaking architecture:

1. **Flaro Relay (`flaro-relay`)**: A neutral Node.js/Socket.io server that acts as a matchmaking lobby. It routes WebRTC handshake signals (`offer`, `answer`, `ice-candidates`) between the desktop and mobile client using a secure 6-digit PIN.
2. **Flaro Desktop (`flaro-desktop`)**: An Electron app running on the host PC. It generates a secure PIN, connects to the Relay, captures the desktop screen, and executes simulated mouse/keyboard events via `nut-js`.
3. **Flaro Web (`flaro-web`)**: A React web application served to the mobile device. Users enter the PIN to join the Relay room, which establishes a direct Peer-to-Peer WebRTC tunnel. It displays the live video feed and captures touch inputs.
4. **Flaro Shared (`flaro-shared`)**: A shared library containing common TypeScript types and Zod schemas used across all projects.
...
To launch the entire Flaro ecosystem on your local machine for development and testing, simply run:

```bash
node start-all.js
```

*(This will automatically boot `flaro-relay` on port 3000, `flaro-web` on port 5173, and subsequently launch the Flaro Electron Desktop Application).*
...
To create a new release, simply open a terminal in the root `flaro` directory and run:
...
3. Packages `flaro-desktop` into a standalone Windows `.exe` using `electron-builder`.
